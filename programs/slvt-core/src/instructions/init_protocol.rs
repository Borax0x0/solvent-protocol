...
pub fn handler(ctx: Context<InitProtocol>, liquidity_buffer_bps: u16) -> Result<()> {
    ...
    ctx.accounts.vault_config.slvt_mint = ctx.accounts.slvt_mint.key();
    ctx.accounts.vault_config.sslvt_mint = ctx.accounts.sslvt_mint.key();
    ...
    ctx.accounts.staking_state.sslvt_exchange_rate = 1_000_000_000_000;
    ctx.accounts.staking_state.total_staked_slvt = 0;

...
pub fn handler(
    ...
    require!(
        new_exchange_rate >= ctx.accounts.staking_state.ssusd_exchange_rate,
        ErrorCode::RateCannotDecrease
    );

    let old_exchange_rate = ctx.accounts.staking_state.ssusd_exchange_rate;

    ctx.accounts.staking_state.ssusd_exchange_rate = new_exchange_rate;
